...
#include "alg/mgcbs_w2/mgcbs_low_w2.hpp"
namespace mg_cbs_w2 {
struct MiddleState {
  MiddleState() = default;
  MiddleState(int _goal_state, int _target_id)
...
template <>
struct std::hash<mg_cbs_w2::MiddleState> {
  size_t operator()(const mg_cbs_w2::MiddleState &x) const {
    size_t seed = 0;
...
class MGCBSMid {
 public:
  struct MiddleLevelNode {
    ...
    MiddleLevelNode(int state, int target_id, int g, int f)
        : middle_state(state, target_id), g(g), f(f) {}
  ...
  bool Search(const Agent &agent, const HTable &h_table,
    ...
    auto handle =
        open_set.push(MiddleLevelNode(0, 0, 0, h_table.GetHeuristicMid(0, 0)));
    state_to_heap[MiddleState(0, 0)] = handle;
    ...
    while (!open_set.empty()) {
      ...
      while (remain_state > 0) {
        ...
        auto np = MiddleState(next_state, next_target_id);
        {
        ...
        }
        {
          ...
          auto next_tis_state = TISState(Interval(0, Interval::oo),
                                         agent.targets[next_target_id].x,
                                         agent.targets[next_target_id].y);
          if (next_state == all_goal_state) {
            next_tis_state = segments[next_target_id].back();
          }
          next_g = low_level_search_.Search(
              current_ts_state, next_tis_state, cons,
          ...
          if (iter == state_to_heap.end()) {
            auto handle = open_set.push(MiddleLevelNode(
                next_state, next_target_id, next_g,
                next_g + h_table.GetHeuristicMid(next_state, next_target_id)));
