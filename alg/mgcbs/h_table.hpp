...
template <>
struct std::hash<mg_cbs::HeuristicState> {
  size_t operator()(const mg_cbs::HeuristicState &x) const {
    size_t seed = 0;
...
class HTable {
 public:
  void Build(const std::vector<std::vector<int>> &distance_matrix) {
    Build_MST(distance_matrix);
  }
  int GetHeuristic(int state, int target_id) const {
 ...
 private:
  ...
  // means the goal have been reached. The goal of agent staying in is 1.
  // j: the agent is in goal j.
