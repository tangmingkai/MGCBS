...
class DistanceMatrixBuilder {
 public:
  ...
  void BuildDistanceMatrix(
    ...
    // target_distance_matrix[j][i]: The distance from agent.targets[i]
    //                               to agent.targets[j]
    ...
    for (int i = 1; i < static_cast<int>(agent.targets.size()); i++) {
      // full_map_distance[x][y]: The distance from (x,y) to agent.targets[i]
      ...
      for (uint j = 0; j < neighbors.size(); j++) {
        ...
        if ((*full_map_distance)[neighbour.location.x][neighbour.location.y] >
          ...
          (*full_map_distance)[neighbour.location.x][neighbour.location.y] =
              current_c + 1;
          // std::cout << "Update:" << neighbour.location.x << " "
          // <<neighbour.location.y << " "<< (current_c + 1) << std::endl;
